...
plot_modX <-
  function(x,x2,x3,chr,lodcolumn=1,incl.markers=TRUE,xlim, ylim, cex.x,
    ...
    if(!add && !onechr) {
      if(!alternate.chrid || length(xtick) < 2) {
        for(i in seq(along=xtick))
          axis(side=1, at=xtick[i], labels=xticklabel[i], cex.axis = cex.x)
      }
      else {
        odd <- seq(1, length(xtick), by=2)
        even <- seq(2, length(xtick), by=2)
        for(i in odd) {
          axis(side=1, at=xtick[i], labels="", cex.axis = 1)
          axis(side=1, at=xtick[i], labels=xticklabel[i], line=-0.4, tick=FALSE, cex.axis = cex.x) # changed line from -0.2 to -0.4
        }
        for(i in even) {
          axis(side=1, at=xtick[i], labels="", cex.axis = 1)
          axis(side=1, at=xtick[i], labels=xticklabel[i], line=+0.3, tick=FALSE, cex.axis = cex.x) # changed line from +0.6 to +0.2
        }
      }
    }
