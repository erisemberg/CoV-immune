...
aac_p2
ggsave("~/Documents/ValdarFerris/defense/figures/aac.png", width = 5.5)
...
# Flow cytometry phenotypes were grouped by cellular lineage and biological interpretation, 
# including abundance, activation state, trafficking, and functional measures. Activation 
# state included markers of both acute activation and inhibitory signaling (e.g., CD25, CD69, PD-1).
...
ggsave("figures/supplemental/phenos.png", width = 11, height = 6, bg = "white")

# plot1 <- plot_grid(pheno_desc, NULL, vc_plot, ncol = 3, rel_widths = c(0.4,0.05,0.55), align = "h", labels = c("A", "B"))
# plot2 <- plot_grid(plot1, plot_grid(NULL, pca_plot, ncol = 2, rel_widths = c(0.1,0.9)), 
#                    nrow = 2, rel_heights = c(0.45,0.55), labels = c("", "C"))
# plot_grid(plot2, hmgrob, ncol = 2, rel_widths = c(0.55, 0.45), labels = c("", "D"))

# fig2 <- plot_grid(plot_grid(pheno_desc, NULL, NULL, vc_plot, NULL, ncol = 5, align = "h", 
#                     rel_widths = c(0.4,0.05,0.05,0.5,0.05), labels = c("A", "", "B", "", "")),
#           NULL,
#           plot_grid(hmgrob, pca_plot, ncol = 2, rel_widths = c(0.45,0.55), labels = c("C", "D")),
#           nrow = 3, rel_heights = c(0.4,0.03,0.57))
# ggsave("figures/Figure2.png", width = 10, height = 8, bg = "white")
...
range(cross_data$Titer, na.rm = TRUE)
t.test(Titer ~ infection, cross_data, var.equal = FALSE)

summary(lm(weight_aac ~ infection + sex + Titer, cross_data))
summary(lm(HS ~ infection + sex + Titer, cross_data))
...
# plot(log(cross_data$Titer+1), cross_data$weight_aac)
...
sum(VHdf$infectionVH < 0.05) 
print(paste("variance is greater in infected mice for", inf_var_g, "phenotypes."))
...
write_csv(imp_df, "flow/variable_selection/glmnet_variable_importance.csv")
...
# MHCII+ Neutrophils are so important?
boxdf <- data.frame(infection = flow_wInf$infection,
                    MHCII_Neutrophils = flow_wInf$G_Neutrophils_pctMHCII_imp)
ggplot(data = boxdf, aes(x = as.factor(infection), y = MHCII_Neutrophils)) + 
...
X_train <- as.matrix(select(train_data, -infection))
y_train <- as.numeric(train_data$infection) - 1  # XGBoost needs numeric labels
...
# Predict
X_test <- as.matrix(select(test_data, -infection))
y_test <- as.numeric(test_data$infection) - 1

pred_probs <- predict(xgb_model, X_test)
pred_matrix <- matrix(pred_probs, ncol = 3, byrow = TRUE)
pred_labels <- max.col(pred_matrix) - 1

confusionMatrix(factor(pred_labels, levels = 0:2, labels = levels(immune_data$infection)),
                test_data$infection)



# -----------------------------descriptive flow------------------------------- #
ggplot(cross$pheno, aes(x = infection, y = L_DNTCells_pctCentMem, fill = infection)) + 
  geom_boxplot() + 
  scale_x_discrete(limits = names(inf_pal), labels = grouplabels[names(inf_pal)]) + 
  scale_fill_manual(values = inf_pal, breaks = names(inf_pal), labels = grouplabels[names(inf_pal)])

mean(cross$pheno$L_DNTCells_pctCentMem[cross$pheno$infection == "PBS"], na.rm = TRUE)
mean(cross$pheno$L_DNTCells_pctCentMem[cross$pheno$infection == "SARSCoV"], na.rm = TRUE)

# tissue-resident AMs 
trams <- ggplot(cross$pheno, aes(x = infection, y = G_AlveolarMacs_pctofCD45, fill = infection)) + 
  geom_boxplot(outliers = FALSE) + 
  scale_x_discrete(limits = names(inf_pal), labels = grouplabels[names(inf_pal)]) + 
  scale_fill_manual(values = inf_pal, breaks = names(inf_pal), labels = grouplabels[names(inf_pal)]) +
  labs(x = NULL, y = NULL, title = "Alveolar macrophages (%)") +
  theme_minimal() +
  theme(axis.text = element_text(size = 12),
        plot.title = element_text(size = 14),
        legend.position = "none")
fit <- aov(G_AlveolarMacs_pctofCD45 ~ infection, data = cross_data)

tk <- TukeyHSD(fit)$infection %>%
  as.data.frame() %>%
  rownames_to_column("comparison") %>%
  separate(comparison, into = c("group2", "group1"), sep = "-", remove = FALSE) %>%
  rename(p.adj = `p adj`) %>%
  mutate(p.signif = case_when(p.adj < 0.001 ~ "***",
                              p.adj < 0.01  ~ "**",
                              p.adj < 0.05  ~ "*",
                              TRUE          ~ "ns"))
gb <- ggplot_build(trams)
whisker_max <- max(gb$data[[1]]$ymax, na.rm = TRUE)
whisker_min <- min(gb$data[[1]]$ymin, na.rm = TRUE)
y_max <- whisker_max
step <- 0.08 * (whisker_max - whisker_min)
tk <- tk %>%
  arrange(p.adj) %>%
  mutate(y.position = y_max + step * row_number())
trams <- trams + 
  stat_pvalue_manual(tk, label = "p.signif", xmin = "group1", xmax = "group2", 
                     y.position = "y.position", tip.length = 0.01, inherit.aes = FALSE)
trams
ggsave("~/Documents/ValdarFerris/defense/figures/immune_response/TR_AMs.png", width = 4, height = 3)

# ggplot(cross$pheno, aes(x = infection, y = M_AlveolarMacs_pctofCD45, fill = infection)) + 
#   geom_boxplot() + 
#   scale_x_discrete(limits = names(inf_pal), labels = grouplabels[names(inf_pal)]) + 
#   scale_fill_manual(values = inf_pal, breaks = names(inf_pal), labels = grouplabels[names(inf_pal)])

# inflammatory monocytes 
inf_monos <- ggplot(cross$pheno, aes(x = infection, y = M_Ly6Cp_MonoMac_pctofCD45, fill = infection)) + 
  geom_boxplot(outliers = FALSE) + 
  scale_x_discrete(limits = names(inf_pal), labels = grouplabels[names(inf_pal)]) + 
  scale_fill_manual(values = inf_pal, breaks = names(inf_pal), labels = grouplabels[names(inf_pal)]) +
  labs(x = NULL, y = NULL, title = "Inflammatory monocytes (%)") +
  theme_minimal() +
  theme(axis.text = element_text(size = 12),
        plot.title = element_text(size = 14),
        legend.position = "none")
fit <- aov(M_Ly6Cp_MonoMac_pctofCD45 ~ infection, data = cross_data)
tk <- TukeyHSD(fit)$infection %>%
  as.data.frame() %>%
  rownames_to_column("comparison") %>%
  separate(comparison, into = c("group2", "group1"), sep = "-", remove = FALSE) %>%
...
inf_monos <- inf_monos + 
  stat_pvalue_manual(tk, label = "p.signif", xmin = "group1", xmax = "group2", 
                     y.position = "y.position", tip.length = 0.01, inherit.aes = FALSE)
inf_monos
ggsave("~/Documents/ValdarFerris/defense/figures/immune_response/inf_monos.png", width = 4, height = 3)


# T cells - decrease in human, more pronounced in CD8+ compartment and particularly prominent in severe disease 
cytoT <- ggplot(cross$pheno, aes(x = infection, y = L_CytoTCells_pctofCD45, fill = infection)) + 
  geom_boxplot(outliers = FALSE) + 
  scale_x_discrete(limits = names(inf_pal), labels = grouplabels[names(inf_pal)]) + 
  scale_fill_manual(values = inf_pal, breaks = names(inf_pal), labels = grouplabels[names(inf_pal)]) +
  labs(x = "Infection", y = "Cytotoxic T cells (%)") +
  theme_minimal() +
  theme(axis.text = element_text(size = 12),
        axis.title = element_text(size = 12),
        legend.position = "none")
fit <- aov(L_CytoTCells_pctofCD45 ~ infection, data = cross_data)
tk <- TukeyHSD(fit)$infection %>%
  as.data.frame() %>%
  rownames_to_column("comparison") %>%
  separate(comparison, into = c("group2", "group1"), sep = "-", remove = FALSE) %>%
...
cytoT <- cytoT + 
  stat_pvalue_manual(tk, label = "p.signif", xmin = "group1", xmax = "group2", 
                     y.position = "y.position", tip.length = 0.01, inherit.aes = FALSE)
cytoT
ggsave("~/Documents/ValdarFerris/defense/figures/immune_response/cytoTcells.png", width = 4, height = 3)

helpT <- ggplot(cross$pheno, aes(x = infection, y = L_HelpTCells_pctofCD45, fill = infection)) + 
  geom_boxplot(outliers = FALSE) + 
  scale_x_discrete(limits = names(inf_pal), labels = grouplabels[names(inf_pal)]) + 
  scale_fill_manual(values = inf_pal, breaks = names(inf_pal), labels = grouplabels[names(inf_pal)]) +
  labs(x = "Infection", y = "Helper T cells (%)") +
  theme_minimal() +
  theme(axis.text = element_text(size = 12),
        axis.title = element_text(size = 12),
        legend.position = "none")
fit <- aov(L_HelpTCells_pctofCD45 ~ infection, data = cross_data)
tk <- TukeyHSD(fit)$infection %>%
  as.data.frame() %>%
  rownames_to_column("comparison") %>%
  separate(comparison, into = c("group2", "group1"), sep = "-", remove = FALSE) %>%
...
helpT <- helpT + 
  stat_pvalue_manual(tk, label = "p.signif", xmin = "group1", xmax = "group2", 
                     y.position = "y.position", tip.length = 0.01, inherit.aes = FALSE)
helpT
ggsave("~/Documents/ValdarFerris/defense/figures/immune_response/helpTcells.png", width = 4, height = 3)

# B cells
ggplot(cross$pheno, aes(x = infection, y = L_BCells_pctofCD45, fill = infection)) + 
  geom_boxplot() + 
  scale_x_discrete(limits = names(inf_pal), labels = grouplabels[names(inf_pal)]) + 
  scale_fill_manual(values = inf_pal, breaks = names(inf_pal), labels = grouplabels[names(inf_pal)]) +
  labs(x = "Infection", y = "B cells (%)") +
  theme_minimal() +
  theme(axis.text = element_text(size = 12),
        axis.title = element_text(size = 12),
        legend.position = "none")

# NK cells - decrease in humans, not recapitulated here 
ggplot(cross$pheno, aes(x = infection, y = L_NKCells_pctofCD45, fill = infection)) + 
  geom_boxplot() + 
  scale_x_discrete(limits = names(inf_pal), labels = grouplabels[names(inf_pal)]) + 
  scale_fill_manual(values = inf_pal, breaks = names(inf_pal), labels = grouplabels[names(inf_pal)]) +
  labs(x = "Infection", y = "NK cells (%)") +
  theme_minimal() +
  theme(axis.text = element_text(size = 12),
        axis.title = element_text(size = 12),
        legend.position = "none")

# neutrophils - inc. in humans, not recapitulated here 
ggplot(cross$pheno, aes(x = infection, y = G_Neutrophils_pctofCD45, fill = infection)) + 
  geom_boxplot() + 
  scale_x_discrete(limits = names(inf_pal), labels = grouplabels[names(inf_pal)]) + 
  scale_fill_manual(values = inf_pal, breaks = names(inf_pal), labels = grouplabels[names(inf_pal)]) +
  labs(x = "Infection", y = "Neutrophils (%)") +
  theme_minimal() +
  theme(axis.text = element_text(size = 12),
        axis.title = element_text(size = 12),
        legend.position = "none")

# eosinophils - inc. in humans, not recapitulated here
ggplot(cross$pheno, aes(x = infection, y = G_Eosinophils_pctofCD45, fill = infection)) + 
  geom_boxplot() + 
  scale_x_discrete(limits = names(inf_pal), labels = grouplabels[names(inf_pal)]) + 
  scale_fill_manual(values = inf_pal, breaks = names(inf_pal), labels = grouplabels[names(inf_pal)]) +
  labs(x = "Infection", y = "Eosinophils (%)") +
  theme_minimal() +
  theme(axis.text = element_text(size = 12),
        axis.title = element_text(size = 12),
        legend.position = "none")

# pDCs - dec. early after infection, not recapitulated here
ggplot(cross$pheno, aes(x = infection, y = M_pDC_pctofCD45, fill = infection)) + 
  geom_boxplot(outliers = FALSE) + 
  scale_x_discrete(limits = names(inf_pal), labels = grouplabels[names(inf_pal)]) + 
  scale_fill_manual(values = inf_pal, breaks = names(inf_pal), labels = grouplabels[names(inf_pal)]) +
  labs(x = "Infection", y = "Plasmacytoid DCs (%)") +
  theme_minimal() +
  theme(axis.text = element_text(size = 12),
        axis.title = element_text(size = 12),
        legend.position = "none")
...
# -------------------------------illustrate VH-------------------------------- #
ggplot(cross$pheno, aes(x = infection, y = G_AlveolarMacs_pctofCD45, fill = infection)) + 
  geom_boxplot(outliers = FALSE) + 
  scale_x_discrete(limits = names(inf_pal), labels = grouplabels[names(inf_pal)]) + 
  scale_fill_manual(values = inf_pal, breaks = names(inf_pal), labels = grouplabels[names(inf_pal)]) +
  labs(x = NULL, y = NULL, title = "Alveolar macrophages (%)") +
  ...
  theme(axis.text = element_text(size = 12),
        ...
        plot.title = element_text(size = 14),
        legend.position = "none")
ggsave("~/Documents/ValdarFerris/defense/figures/chapter3/VH_AMs.png", width = 4, height = 3)

# CD69+ cytotoxic T cells variance heterogeneity 
ggplot(cross$pheno, aes(x = infection, y = L_CytoTCells_pctCD69, fill = infection)) + 
  geom_boxplot(outliers = FALSE) + 
  scale_x_discrete(limits = names(inf_pal), labels = grouplabels[names(inf_pal)]) + 
  scale_fill_manual(values = inf_pal, breaks = names(inf_pal), labels = grouplabels[names(inf_pal)]) +
  labs(x = "Infection", y = expression("Cytotoxic T cells (% CD69"^"+"*")")) +
  theme_minimal() +
  theme(axis.text = element_text(size = 12),
        axis.title = element_text(size = 13),
        legend.position = "none")
ggsave("~/Documents/ValdarFerris/defense/figures/chapter3/VH_CD69cytoT.png", width = 4, height = 3)
...
# are MHCII+ IMs lower in SARS-CoV-2? 
aov_res <- aov(formula = M ~ infection, data = df)
summary(aov_res)
TukeyHSD(aov_res)
# plot 
ggplot(data = df, mapping = aes(x = infection, y = M, fill = infection)) + 
  geom_boxplot(outliers = FALSE) + 
  scale_fill_manual(values = inf_pal, name = "Infection") +
  theme_minimal()
