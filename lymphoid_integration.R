...
# plot residuals (KJ1 vs KJ2 or KJ1 vs KK) and save betas/p-vals and look for 
# especially problematic phenotypes
res <- data.frame(phenotype = character(),
                  beta = numeric(),
                  pval = numeric(), 
                  comparison = character())
res.ix <- 0

gater <- all_lymph_og$Gater

par(mfrow=c(2,2))
for(pheno in phenotypes){
  if(length(unique(gater[which(!is.na(all_lymph[,pheno]))])) > 1){
    dat <- all_lymph %>% dplyr::select(pheno, 'DATE_SAMPLE', 'Gater')
    dat <- dat %>% pivot_wider(id_cols = 'DATE_SAMPLE', names_from = 'Gater', values_from = pheno)
    
    if (sum(!is.na(dat$KJ1)) > 0){ # if there is data in KJ1 dataset
      if (sum(!is.na(dat$KJ2)) > 0){ # if there is data in KJ2 dataset 
        
        plot(dat$KJ1, dat$KJ2, col='red', main = pheno) # plot Kara first gating vs second gating 
        abline(a=0,b=1)
        mod <- lm(KJ2 ~ KJ1, dat)
        res.ix <- res.ix + 1
        res[res.ix,] <- c(pheno,
                          summary(mod)$coefficients['KJ1', 'Estimate'], 
                          summary(mod)$coefficients['KJ1', 'Pr(>|t|)'],
                          'KJ1 vs KJ2')
        
        if (sum(!is.na(dat$KK)) > 0){
          points(dat$KJ1, dat$KK, col = 'blue') # plot Kara first gating vs Kalika 
          mod <- lm(KK ~ KJ1, dat)
          res.ix <- res.ix + 1
          res[res.ix,] <- c(pheno,
                            summary(mod)$coefficients['KJ1', 'Estimate'], 
      ...
      } else if (sum(!is.na(dat$KK)) > 0) { # No data in KJ2 dataset, data in KK dataset 
        plot(dat$KJ1, dat$KK, col='blue', main = pheno) # plot Kara first gating vs Kalika 
        abline(a=0,b=1)
        mod <- lm(KK ~ KJ1, dat)
        res.ix <- res.ix + 1
        res[res.ix,] <- c(pheno, 
    ...
    print(paste0(pheno, " does not have multiple measurements."))
  }
}
...
# phenotypes indicate variation between gatings of the same file - for example: 
#   BCells_pctCD25: KJ1 appears to be higher than KJ2 and KK
#   BCells_pctPD1: outliers at higher values
#   CytoTCells_pctofCD3: outliers throughout 
#   CytoTCells_pctPD1: outliers at higher values 
#   HelpTCells_pctCCR7: significant positive slope for KK vs KJ1 ($\beta = 8$)
